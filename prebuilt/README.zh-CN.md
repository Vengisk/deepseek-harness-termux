...
- `patch`(`pkg install patch`)——用于应用源码补丁
...
# 3. 把 dsh-termux.tgz 上传到 GitHub Release,用户即可从上面的
#    releases/latest/download 地址安装
```
...
- `dsh` 命令(由 `@deepseek-ai/dsh` 自身安装)通过修改后的 shebang 自动带上
  `--expose-internals`。
